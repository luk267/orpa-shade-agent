...
function matchWatchlist(tx: any, wl: Watchlist) {
...
function upsertTx(db: Database.Database, tx: {
  hash: string,
  block_height: number,
...
export async function scanRecentBlocks(nBlocks: number) {
  const db = openDb();
  ...
  for (let h = start; h <= tipHeight; h++) {
    ...
    for (const chunk of b.chunks) {
      ...
      for (const tx of txs) {
        if (!matchWatchlist(tx, wl)) continue;
    ...
    if (rowsToInsert.length) insert(rowsToInsert);
